 ...
 * Mirrors Qualcomm htp_op_desc layout: src_idx[HTP_OP_MAX_INPUTS=10] + dst_idx[HTP_OP_MAX_OUTPUTS=4]. */
typedef struct hex_op_desc {
    ...
    int32_t src_idx[10];      /* indices into tensor table (-1 = none); mirrors htp_op_desc.src[HTP_OP_MAX_INPUTS] */
